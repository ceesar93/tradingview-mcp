...
**Hard-kill reliability against a frozen process (fixed 2026-07-16):** both `launch_tv_debug_mac.sh` and `tv_launch`'s `kill_existing: true` path previously sent a single `pkill`/SIGTERM and only `sleep`'d a fixed 1-1.5s before relaunching, with no check that the old process actually exited. A hung/frozen TradingView Desktop process can outlive a plain SIGTERM — reproduced live: the script reported success while `ps aux` showed the original PID still running with unchanged uptime, and the "relaunch" just hit Electron's single-instance lock, spawning throwaway helper processes while the frozen instance survived untouched. Both now poll for the process's actual death (up to ~5s) and escalate to `kill -9`/SIGKILL on any survivor before proceeding to relaunch. This does not change the separate chart-tab-restoration gotcha above — a verified-dead-before-relaunch fix doesn't make TradingView any more likely to reopen its last chart tab.
